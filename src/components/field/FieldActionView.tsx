...
import { Smartphone, MapPin, CheckCircle2, Clock, ShieldCheck, AlertCircle, Plus, RefreshCw, Camera } from 'lucide-react';
...
export const FieldActionView: React.FC<FieldActionViewProps> = ({
  ...
  // Encontra a equipe e o ponto de ação designado para este responsável de campo
  const myTeam = teams.find((t) => t.id === currentUser.teamId) || teams[0];
  const assignedPoints = actionPoints.filter((p) =>
    currentUser.assignedActionPointIds?.includes(p.id) || myTeam?.assignedPointIds.includes(p.id)
  );
  const myCheckIns = checkIns.filter(
    (c) => c.coordinatorId === currentUser.id || c.teamId === myTeam?.id
  );
    ...
    <div className="max-w-md mx-auto px-4 py-6 space-y-5 font-['Inter',sans-serif]">
      ...
      <div className="bg-gradient-to-br from-slate-900 via-slate-900 to-emerald-950/40 p-5 rounded-2xl border border-emerald-500/30 shadow-xl space-y-4">
        ...
        <div className="bg-slate-950/70 p-3.5 rounded-xl border border-slate-800 space-y-1.5">
          <div className="text-[11px] font-semibold text-slate-400 uppercase flex items-center gap-1.5">
            ...
            <span>Ponto de Ação Designado</span>
          </div>
          <div className="text-sm font-bold text-white">
            {assignedPoints[0]?.name || 'Ponto de Ação Principal'}
          </div>
          <div className="text-xs text-slate-400">
            {assignedPoints[0]?.address || 'Endereço registrado para a campanha'}
          </div>
        </div>
      </div>
      ...
      {!showCheckInFlow ? (
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 shadow-2xl">
          <CheckInFlow
            team={myTeam}
            actionPoints={assignedPoints.length > 0 ? assignedPoints : actionPoints}
            ...
            {myCheckIns.map((chk) => (
              ...
              >
                ...
                {chk.imageWatermarkUrl && (
                  <div className="relative rounded-lg overflow-hidden border border-slate-800 h-32">
                    <img
